...
func (c *Client) GetUpload(uploadID string) (ApiResult[UploadStatusPublic], error) {
	out := ApiResult[UploadStatusPublic]{}
	err := c.agent.Query(
		c.canisterID, "getUpload",
		[]any{uploadID},
		[]any{&out},
	)
	return out, err
...
func (c *Client) CancelUpload(uploadID string) (resultUnit, error) {
	out := resultUnit{}
	err := c.agent.Call(
		c.canisterID, "cancelUpload",
		[]any{uploadID},
		[]any{&out},
	)
	return out, err
